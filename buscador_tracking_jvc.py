...
import cv2, numpy as np, time, threading, socket, struct, json, traceback
...
Kp_deg_per_err, MAX_SPEED_DEG_S = 0.08, 4.0
...
def detect_brightest(frm, prev_xy=None, lock_radius=LOCK_RADIUS_PX):
    ...
    for c in cnts:
        ...
        if prev_xy is not None:
            dx = cx-prev_xy[0]; dy = cy-prev_xy[1]
            if (dx*dx+dy*dy)>(lock_radius*lock_radius): continue
        cand.append((area, cx, cy))
    if not cand: return None, bw
    cand.sort(reverse=True, key=lambda t:t[0])
    _, cx, cy = cand[0]; return (cx,cy), bw

def compute_command_from_error(dx_px, dy_px):
    v_az = np.clip(SIGN_AZ * (-dx_px * deg_per_px[0] * Kp_deg_per_err), -MAX_SPEED_DEG_S, MAX_SPEED_DEG_S)
    v_el = np.clip(SIGN_EL * (+dy_px * deg_per_px[1] * Kp_deg_per_err), -MAX_SPEED_DEG_S, MAX_SPEED_DEG_S)
    return float(v_az), float(v_el)
...
def controller_loop():
    ...
    while True:
        try:
            ...
            h,w = frm.shape[:2]; cx,cy=w//2,h//2
            xy,_ = detect_brightest(frm, prev_xy=ema_xy if track_enabled else None)
            if xy is not None:
                if ema_xy is None: ema_xy=(float(xy[0]),float(xy[1]))
                else: ema_xy=(EMA_ALPHA_TARGET_PX*xy[0]+(1-EMA_ALPHA_TARGET_PX)*ema_xy[0],
                              EMA_ALPHA_TARGET_PX*xy[1]+(1-EMA_ALPHA_TARGET_PX)*ema_xy[1])
            else:
                if ema_xy is not None: ema_xy=(0.95*ema_xy[0]+0.05*cx,0.95*ema_xy[1]+0.05*cy)
            if track_enabled and ema_xy is not None:
                dx=ema_xy[0]-cx; dy=ema_xy[1]-cy
                v_az,v_el=compute_command_from_error(dx,dy)
                _send_rate_cmd(v_az,v_el,1.0,1)
            n+=1
            if time.time()-last>1.0:
                print(time.strftime("[%H:%M:%S]"), f"[CTRL] track={track_enabled} ema={ema_xy} dpx={deg_per_px} signs=({SIGN_AZ},{SIGN_EL}) iter/s~{n}", flush=True)
                last=time.time(); n=0
        except Exception as e:
            print("[CTRL][ERR]", e, flush=True); time.sleep(0.05)
...
PRA, PRB = 0xFF, 0xFA
UDP_TARGET_IP   = "192.168.1.100"   # CAMBIAR a tu Teensy/PLC si no es broadcast
...
Kp_deg_per_err, MAX_SPEED_DEG_S = 0.08, 4.0
EMA_ALPHA_TARGET_PX = 0.35
FALLBACK_DPX = (0.03, 0.03)   # (H, V) deg/pixel fallback si no hay calib
JOY_DEG_PER_UNIT    = 1.0
SIGN_AZ, SIGN_EL    = -1, +1

# Persistencia de calib por zoom (opcional)
...
def get_deg_per_px(v):
    if v is None: return FALLBACK_DPX
    d = CALIB_LUT.get(f"{v:.2f}")
    if not d: return FALLBACK_DPX
...
if UDP_BROADCAST: udp_tx.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
def _dest(): return ("255.255.255.255", UDP_TARGET_PORT) if UDP_BROADCAST else (UDP_TARGET_IP, UDP_TARGET_PORT)

def send_udp_packet(pkt, tag=""):
    try:
        udp_tx.sendto(pkt, _dest())
        if tag: print(time.strftime("[%%H:%%M:%%S]"), f"[UDP-TX:{tag}] {len(pkt)}B -> {_dest()}", flush=True)
    except Exception as e:
        print("[UDP-TX][ERR]", e, flush=True)

def pkt_joy_units(x_units, y_units, throttle, trigger):
    # MISMO FORMATO que tu buscador: ints *100 + throttle + rate
    ...
    while True:
        try:
            ...
            if pkt_id==PACKET_ID_ANGLE and len(data)>=12:
                ...
                with telemetry_lock: telemetry["az"]=float(az); telemetry["el"]=float(el)
                ...
                if plen>=16 and len(data)>=4+plen+1:
                    ...
                    if (sum(payload)&0xFF)==chk:
                        ...
                        with telemetry_lock:
                            telemetry["wcmd_az"]=float(wcmd_az); telemetry["wcmd_el"]=float(wcmd_el)
                            telemetry["wmeas_az"]=float(wmeas_az); telemetry["wmeas_el"]=float(wmeas_el)
            if time.time()-last>2.0:
                with telemetry_lock:
                    print(time.strftime("[%%H:%%M:%%S]"), f"[UDP-RX] Az/El={telemetry['az']}/{telemetry['el']} wmeas={telemetry['wmeas_az']}/{telemetry['wmeas_el']} pkts/2s={cnt}", flush=True)
                last=time.time(); cnt=0
        except Exception as e:
...
class JVCCapture:
    def __init__(self, dev):
        ...
        if not self.cap.isOpened(): raise RuntimeError(f"Cannot open video device {dev}")
    ...
    def _loop(self):
        ...
        while self.running:
            ...
            if ok:
                ...
                cnt += 1
                if cnt % 120 == 0:
                    fps = cnt/max(1e-3, time.time()-t0); print(time.strftime("[%%H:%%M:%%S]"), f"[CAM] fps~{fps:.1f}", flush=True)
            else:
                time.sleep(0.01)
    def read(self):
        with self.lock:
    ...
    def stop(self):
        self.running=False; time.sleep(0.1); self.cap.release(); print("[CAM] stopped", flush=True)
...
def mjpeg_generator():
    print("[HTTP] /video_feed start", flush=True)
    last=time.time(); sent=0
    while True:
        try:
            frm = cam.read()
            if frm is None:
            ...
            sent += 1
            if time.time()-last>2.0:
                print(time.strftime("[%%H:%%M:%%S]"), f"[HTTP] mjpeg tx ~{sent/ max(1e-3, (time.time()-last)):.1f} fps to client", flush=True)
                last=time.time(); sent=0
        except GeneratorExit:
            print("[HTTP] video client gone", flush=True); break
        except Exception as e:
            print("[HTTP][ERR] video_feed loop:", e, flush=True); time.sleep(0.1)
...
def status():
    with telemetry_lock:
        s=dict(track=track_enabled, dpx=[deg_per_px[0],deg_per_px[1]], signs=[SIGN_AZ,SIGN_EL], az=telemetry["az"], el=telemetry["el"])
    return jsonify(s)
...
def toggle_tracking():
    ...
    if not track_enabled:
        send_udp_packet(pkt_joy_units(0.0,0.0,1.0,1), "stop")
...
def set_zoom():
    ...
    if v is None: return "Bad payload", 400
    if not (0.0 <= v <= 3.0): return "voltage out of range [0..3]", 400
    zoom_voltage = v; deg_per_px = get_deg_per_px(zoom_voltage)
...
def set_device():
    ...
    try:
        ...
        cam_new = JVCCapture(VIDEO_DEVICE)
        cam = cam_new
...
# -------- Socket.IO: joystick --------
@socketio.on("connect")
def on_connect(): print("[IO] connect", flush=True)
@socketio.on("disconnect")
def on_disconnect(): print("[IO] disconnect", flush=True)

@socketio.on("joystick_update")
def handle_joystick_update(data):
    try:
        x = float(data.get("x", 0.0)); y=float(data.get("y", 0.0))
        throttle = float(data.get("throttle", 1.0)); trigger=int(data.get("trigger",1))
        pkt = pkt_joy_units(x, y, throttle, trigger)
        send_udp_packet(pkt, "joy")
        with telemetry_lock:
            telemetry["wcmd_az"], telemetry["wcmd_el"] = x*throttle, y*throttle
    except Exception as e:
        print("[IO][ERR]", e, flush=True); traceback.print_exc()

# -------- Loops --------
def controller_loop():
    # placeholder: si activas tracking con detección real, acá iría.
    while True:
        time.sleep(0.5)

def udp_rx_thread():
    try: udp_rx_loop()
    except Exception as e: print("[UDP-RX][FATAL]", e, flush=True)

def main():
    global cam
    # Threads
    threading.Thread(target=udp_rx_thread, daemon=True).start()
    cam = JVCCapture(VIDEO_DEVICE)
    threading.Thread(target=controller_loop, daemon=True).start()
    print("[HTTP] serving on 0.0.0.0:5010", flush=True)
...
def calibrate():
    ...
    try:
        data = request.get_json(silent=True) or {}
        spd = float(data.get("spd", 1.0))
        dur = float(data.get("dur", 0.8))
