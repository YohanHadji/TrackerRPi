...
def sendTargetToColimator(pointToSendIn):
    ...
    x2 = 1478.6167 + -0.7778 * pointToSendIn.x + 0.0143 * pointToSendIn.y
    y2 = 1349.0330 + 0.0526 * pointToSendIn.x + -0.5521 * pointToSendIn.y
    
    x2 = round(x2, 3)
    y2 = round(y2, 3)
    
    print(f"Sending target to colimator: {x2}, {y2}")

    pointToSend = LightPoint(pointToSendIn.name, pointToSendIn.isVisible, x2, y2, pointToSendIn.age)
