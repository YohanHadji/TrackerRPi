...
def generate_frames():
    ...
    while True:
        ...
        if (input_values["switchFrame"] == 0):
            ...
            cv2.circle(b_frame, (960,540), input_values["lockRadius"], 255, 2)
            ...
            cv2.circle(frame, (960,540), input_values["lockRadius"], (0, 0, 255), 2)
            for point in LightPointArray:
...
def project_point_to_line_and_normalize(x0, y0, xmin=-627, xmax=726):
    # Line parameters
    m = 0.0445
    b = -35.0465
    ...
    # Intersect: m*x + b = m_perp*(x - x0) + y0
    # Solve for x:
    numerator = m_perp * x0 - y0 + b
    denominator = m - m_perp
    x_intersect = -(numerator / denominator)

    # Now find y from original line
    y_intersect = m * x_intersect + b

    # Normalize x between xmin and xmax
    x_normalized = (x_intersect - xmin) / (xmax - xmin)

    return x_normalized, x_intersect, y_intersect
    ...
    while True:
        ...
        if (not firstTimeNoted):
            ...
            if (scanInProgress): 
                ...
                xLine, xI, yI = project_point_to_line_and_normalize(pointToSend.x, pointToSend.y, xmin=-627, xmax=726)
                
                sendAngleToMirror(xLine)

                
                print(xLine, pointToSend.x, xI, pointToSend.y, yI)
                # printFps()
